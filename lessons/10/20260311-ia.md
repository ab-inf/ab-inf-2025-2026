...
Le AI simboliche cercano di imitare la capacità umana di ragionare elaborando _simboli_, cioé stringhe di caratteri che rappresentano concetti del mondo reale, secondo regole esplicite.

Questo approccio si basa quindi sulla manipolazione di rappresentazioni comprensibili dall'essere umano e sull'inferenza logica, resa possibile da motori di inferenza che permettono di ricavare nuovi fatti da basi di conoscenza esistenti.
...
* Potente per risolvere problemi complessi in ambienti ben definiti
* Facile da controllare, correggere e interpretare;
* Permette di costruire modelli molto trasparenti.

- Dipende da una conoscenza predefinita;
- Richiede notevoli sforzi nella sua definizione;
- Non adattabile all’incertezza, come quelli che si trovano nel mondo reale.
...
Le IA sub-simboliche non rappresentano la conoscenza con simboli visibili o comprensibili dall'uomo (come "mela", "albero", "rosso"), ma la distribuiscono attraverso le _connessioni tra elementi che processano le informazioni_ che la costituiscono (come i neuroni in un cervello umano).
...
Questo approccio si basa sull'applicazione di matematica, statistica e analisi dei dati su larga scala per consentire alle macchine di apprendere dall'esperienza. 

La premessa è che l'intelligenza umana sia approssimabile identificando correlazioni tra dati, sbagliando e autoregolandosi.

Questo consente alle macchine di generalizzare sulla base di esempi, gestire incertezze e lavorare con dati non strutturati.
...
Si tratta a tutti gli effenti di una **black box**.
...
* Molto flessibile, scalabile e adattabile a nuovi dati.

* Non è interpretabile, perché non esiste una rappresentazione esplicita e comprensibile per l’uomo di cosa “sa”; 
...
![bg right contain "https://medium.com/towards-data-science/symbolic-vs-subsymbolic-ai-paradigms-for-ai-explainability-6e3982c6948a"](img/1006.png)

### Esempio di approccio sub-simbolico: Machine Learning

L'_**apprendimento automatico**_ (_machine learning_) è un sottoinsieme di IA che impara dai dati e fa previsioni sulla base di quello che ha imparato, in assenza di istruzioni esplicite (cioé simboliche).

Questo tipo di tecnologia è diventata la base di molti prodotti e servizi moderni, che consente a questi di adattarsi nel tempo elaborando dati di addestramento e perfezionando la loro logica interna.

---

<!-- footer: Razavian, N., Knoll, F., & Geras, K. J. (2020, February). Artificial intelligence explained for nonexperts. In Seminars in musculoskeletal radiology (Vol. 24, No. 01, pp. 003-011). Thieme Medical Publishers. https://www.doi.org/10.1055/s-0039-3401041 -->

![bg right contain vertical "https://natureofcode.com/neural-networks/"](img/1009.png)

![bg right contain "https://natureofcode.com/neural-networks/"](img/1010.png)
...
![bg right contain "https://www.w3schools.com/ai/ai_perceptrons.asp"](img/1011.png)

### L'unità della rete neurale: il percettrone

Ogni dato ha un certo valore numerico (x1, x2, x3, ecc.).

Il valore di ogni dato di input viene moltiplicato con un peso W (un altro valore numerico, generalmente tra 0 e 1, che ne rappresenta l'importanza rispetto al risultato finale).
...
![bg right contain "https://www.w3schools.com/ai/ai_perceptrons.asp"](img/1012.png)
...
![bg right "https://unsplash.com/photos/group-of-boy-band-standing-on-stadium-MV7fWnQ391Y"](img/1013.png)
...
* L’artista è bravo? = 0 o 1 (peso = 0.7)
* Il tempo è buono? = 0 o 1 (peso = 0.6)
* Siamo con amici? = 0 o 1 (peso = 0.5)
* Servono cibo? = 0 o 1 (peso = 0.3)
* Servono alcolici? = 0 o 1 (peso = 0.4)
...
Diciamo anche che la nostra soglia da considerare per prendere la decisione sia 1.5: sotto la soglia, l’output è 0 (“non vado al concerto”); sopra la soglia, l’output è 1 (“vado al concerto”).
...
Esiste anche il concetto di **bias**, un peso extra slegato dagli input, una sorta di "assunto di partenza" che viene aggiustato autonomamente per spostare la soglia di decisione in maniera più flessibile e adattabile.

Nella realtà, la decisione non dipende solo dai fattori esterni, ma anche dalla nostra **predisposizione di base** (amiamo andare ai concerti, o preferiamo stare a casa?)

Il **bias** rappresenta matematicamente questa inclinazione, sostituendo la soglia e portando il confronto sempre a zero: `Somma dei pesi + Bias > 0`.
...
Invece di dire "la somma deve superare la soglia di 1.5", trasformiamo la soglia in un bias negativo di **-1.5** (che rappresenta, ad esempio, la naturale fatica di uscire di casa la sera).

Il nuovo calcolo diventa:

* Somma dei valori pesati = 1.6
* Bias (resistenza a uscire) = -1.5

Totale: 1.6 - 1.5 = **0.1**

Dato che 0.1 è maggiore di 0, l'output è 1: andrò al concerto.

---

<!-- footer: Razavian, N., Knoll, F., & Geras, K. J. (2020, February). Artificial intelligence explained for nonexperts. In Seminars in musculoskeletal radiology (Vol. 24, No. 01, pp. 003-011). Thieme Medical Publishers. https://www.doi.org/10.1055/s-0039-3401041 -->

![bg right contain "https://commons.wikimedia.org/w/index.php?curid=24913461"](img/1007.png)

### Reti neurali artificiali

Le _**artificial neural networks**_ sono modelli computazionali composti da gruppi di neuroni artificiali organizzati in livelli.
...
![bg right contain "https://www.edureka.co/blog/what-is-a-neural-network/"](img/1014.png)

### Reti neurali artificiali

Ogni neurone ad ogni livello lavora con un **microtratto o caratteristica** (_feature_), ovvero una singola proprietà misurabile dei dati forniti in input o estratti nei livelli nascosti.
...
Ogni neurone nei layer nascosti prende dei numeri in ingresso (che sono i pixel dell'immagine), li combina con i pesi, e poi applica una funzione di attivazione per determinare se deve attivarsi o meno. Se i numeri passano bruscamente da alti (chiaro) a bassi (scuro), la rete ha rilevato probabilmente un bordo.

I layer nascosti quindi trasformano questi numeri di input in rappresentazioni più astratte. Se rileva un bordo orizzontale che si unisce a uno verticale, ha trovato forse un angolo.
...
All'inizio, i numeri sono del tutto casuali, quindi l'IA non vede nulla e sbaglia a riconoscere l'immagine. Ma, per ridurre questo errore finale, modifica a ritroso i valori, e impara da solo a ridurre il più possibile l'errore.

---

<!-- footer: Razavian, N., Knoll, F., & Geras, K. J. (2020, February). Artificial intelligence explained for nonexperts. In Seminars in musculoskeletal radiology (Vol. 24, No. 01, pp. 003-011). Thieme Medical Publishers. https://www.doi.org/10.1055/s-0039-3401041 -->

![bg right contain ""](img/1015.png)

### Tipi di Machine Learning

L’apprendimento è un processo iterativo che consiste nell’ottimizzazione progressiva dei parametri che definiscono il modello di IA.
...
![bg right contain "http://dx.doi.org/10.3389/fphar.2021.720694"](img/1015.png)
...
![bg right contain "https://www.edureka.co/blog/what-is-a-neural-network/"](img/1016.png)

### Apprendimento supervisionato

In fase di addestramento, vengono forniti i dati su cui allenarsi (es. immagini da classificare a seconda che contengano gatti oppre cani).

Anche la risposta corretta viene fornita all'IA (es. a tutte le immagini di cani, viene associata l’etichetta "cane"; a tutte le immagini di gatti, viene associata l’etichetta "gatto").

Dopo l'addestramento, l'IA è in grado di riconoscere autonomamente gatti e cani nelle foto.

---

<!-- footer: Razavian, N., Knoll, F., & Geras, K. J. (2020, February). Artificial intelligence explained for nonexperts. In Seminars in musculoskeletal radiology (Vol. 24, No. 01, pp. 003-011). Thieme Medical Publishers. https://www.doi.org/10.1055/s-0039-3401041 -->

![bg right contain "https://www.edureka.co/blog/what-is-a-neural-network/"](img/1017.png)

### Apprendimento non-supervisionato

In fase di apprendimento della rete, vengono forniti solo dati senza la risposta corretta.

L'IA individua da sola regolarità statistiche presenti nei dati (es. la conformazione del muso, la dimensione, i colori, ecc.) e li raggruppa in _cluster_ omogenei sulla base di queste regolarità.

Ottimale per i dati grezzi, ma richiede enormi quantità di materiale e di potenza di calcolo.

---

<!-- footer: Razavian, N., Knoll, F., & Geras, K. J. (2020, February). Artificial intelligence explained for nonexperts. In Seminars in musculoskeletal radiology (Vol. 24, No. 01, pp. 003-011). Thieme Medical Publishers. https://www.doi.org/10.1055/s-0039-3401041 -->

![bg right contain "https://en.wikipedia.org/wiki/Reinforcement_learning"](img/1018.png)

### Apprendimento per rinforzo

In fase di apprendimento, l’IA esegue azioni e riceve feedback sotto forma di ricompense o penalizzazioni.

L'IA ha l’obiettivo di imparare a scegliere le azioni che massimizzano la ricompensa complessiva nel lungo periodo.
