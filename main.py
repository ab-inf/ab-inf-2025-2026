...
def main(dirs):
    theme_path = Path("../../theme.css").resolve()
    for dir_name in dirs:
        ...
        for md_file in lesson_path.glob("*.md"):
            ...
            commands = [
                ["npx", "@marp-team/marp-cli@latest", str(md_file), "-o", str(md_file.with_suffix(".html")), "--theme", str(theme_path)],
                ["npx", "@marp-team/marp-cli@latest", str(md_file), "-o", str(md_file.with_suffix(".pptx")), "--theme", str(theme_path), "--allow-local-files"]
            ]
            for cmd in commands:
